...
use crate::core::TypeContext;
use crate::errors::TypeError;
use crate::frontend::parse_module;
use crate::performance::incremental::{IncrementalEngine, ModuleId};
use crate::performance::cache::{ResultCache, CacheKey, CacheEntry, CachedError};
use dashmap::DashMap;
use rayon::prelude::*;
...
impl ParallelAnalyzer {
    ...
    fn analyze_task(&self, task: &AnalysisTask) -> AnalysisResult {
        ...
        let cache_key = CacheKey {
            module: task.id,
            hash: crate::performance::incremental::ContentHash::from_str(&task.content),
        };
        ...
        let errors = match parse_module(&task.content) {
            ...
            Err(e) => {
                vec![crate::analysis::checker::TypeError {
        ...
        let result_errors: Vec<crate::errors::TypeError> = errors.iter().map(|e| {
            crate::errors::TypeError::new(
                crate::errors::ErrorKind::TypeMismatch {
                    expected: "".to_string(),
                    found: e.message.clone(),
                },
                crate::errors::SourceLocation::new(e.line, e.col, e.line, e.col),
            )
        }).collect();
    ...
    fn cached_error_to_type_error(&self, cached: &CachedError) -> TypeError {
        TypeError::new(
            ...
            crate::errors::SourceLocation::new(cached.line, cached.col, cached.line, cached.col),
...
mod tests {
    use super::*;
    use crate::performance::DependencyGraph;
